...
type MindMapHost = CustomViewHost & {
  entities: Entities;
  ai: {
    availability(feature: "chat"): Promise<{ feature: "chat"; available: boolean }>;
...
function App() {
  ...
  const [aiProgressStage, setAiProgressStage] = useState<"preparing" | "generating" | "finishing">("preparing");
  ...
  useEffect(() => {
    let mounted = true;
    void host.ai.availability("chat")
      .then((availability) => { if (mounted) setAiAvailable(availability.available); })
      .catch(() => { if (mounted) setAiAvailable(false); });
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      setAiElapsedSeconds(elapsed);
      if (elapsed >= 2) setAiProgressStage((stage) => stage === "finishing" ? stage : "generating");
    }, 250);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      <header className="workspace-bar">
        ...
        <div className="toolbar-actions">
          ...
          {aiAvailable&&<button className="ai-button" onClick={()=>openAi("new")}><Icon name="sparkles" size={17}/><span>{t("ai")}</span></button>}
    ...
    {aiOpen && <div className="dialog-backdrop" onMouseDown={event=>event.target===event.currentTarget&&!aiBusy&&setAiOpen(false)}><section className="dialog" role="dialog" aria-modal="true" aria-labelledby="ai-title" aria-describedby={aiBusy?"ai-progress-status":undefined}><div className="dialog-head"><span className="dialog-icon"><Icon name="sparkles"/></span><div><h2 id="ai-title">{t("aiTitle")}</h2><p>{aiMode==="new"?t("aiNew"):t("aiExpand")}</p></div><button className="icon-button" aria-label={t("cancel")} disabled={aiBusy} onClick={()=>setAiOpen(false)}><Icon name="close"/></button></div><textarea autoFocus value={aiPrompt} maxLength={4000} disabled={aiBusy} placeholder={aiMode==="new"?t("aiPrompt"):t("aiPromptExpand")} onChange={event=>setAiPrompt(event.target.value)} onKeyDown={event=>{if((event.metaKey||event.ctrlKey)&&event.key==="Enter")void runAi();}}/><p className="ai-hint">{t("aiHint")}</p>{aiBusy&&<div className="ai-progress" id="ai-progress-status" role="status" aria-live="polite"><div className="ai-progress-head"><strong>{t(aiProgressStage==="preparing"?"aiProgressPreparing":aiProgressStage==="finishing"?"aiProgressFinishing":"aiProgressGenerating")}</strong><span>{t("aiProgressElapsed",{seconds:aiElapsedSeconds})}</span></div><div className="ai-progress-track" role="progressbar" aria-label={t("generating")}><span/></div>{aiElapsedSeconds>=10&&<p>{t("aiProgressLong")}</p>}</div>}{aiError&&<p className="dialog-error" role="alert">{aiError}</p>}<div className="dialog-actions"><button className="secondary-button" disabled={aiBusy} onClick={()=>setAiOpen(false)}>{t("cancel")}</button><button className="primary-button" disabled={!aiPrompt.trim()||aiBusy} onClick={()=>void runAi()}>{aiBusy?<span className="spinner"/>:<Icon name="sparkles" size={16}/>} {aiBusy?t("generating"):t("generate")}</button></div></section></div>}
