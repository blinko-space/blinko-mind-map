...
type MindMapHost = CustomViewHost & {
  ...
  ai: {
    ...
    generateStream(input: { prompt: string; system?: string; maxOutputTokens?: number }, onTextDelta: (delta: string) => void): Promise<{ text: string }>;
  };
};
...
function App() {
  ...
  const aiStreamLine = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const line = aiStreamLine.current;
  ...
  const runAi = async () => {
    ...
    setAiBusy(true); setAiError(""); setAiStreamText("");
    try {
      ...
      const result = await host.ai.generateStream({
        prompt: task,
      ...
      }, (delta) => setAiStreamText((text) => (text + delta).slice(-8_000)));
